...
import { Card, Row, Col, Spinner, Badge, ListGroup, Form } from 'react-bootstrap';
...
import { useRatingsStats, useRatingsTimeline, useServicesRatings } from '../hooks/useRatings';
...
export const RatingsDashboard: React.FC = () => {
  ...
  const { stats, loading: statsLoading, error: statsError } = useRatingsStats(selectedService, days);
  const { timeline, loading: timelineLoading } = useRatingsTimeline(selectedService, days);
  ...
  // Distribution Data
  const distributionLabels = Object.keys(stats.distribution).map(k => `${k} Stars`);
  ...
  return (
    <div>
      ...
      <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
        <h4 className="mb-0 text-brand">⭐ {t('title') || 'Ratings & Feedback'}</h4>
        ...
        <Col md={3}>
          <Card className="shadow-sm border-0 h-100 text-center">
            <Card.Body>
              ...
              <div className={`display-4 fw-bold text-${getRatingColor(stats?.averages?.overall || 0)}`}>
        ...
        <Col md={3}>
          <Card className="shadow-sm border-0 h-100 text-center">
            <Card.Body>
              ...
              <div className="display-4 fw-bold">{stats?.total_ratings || 0}</div>
        ...
        <Col lg={7}>
          <Card className="shadow-sm border-0 h-100">
            ...
            <Card.Body>
              <Bar 
                data={{
                  labels: ['Clarity', 'Usefulness', 'Accuracy', 'Usability', 'Speed', 'Design', 'Recommend'],
                  datasets: [{
                    label: t('average'),
                    data: [
      ...
      <Card className="shadow-sm border-0 mb-4">
        ...
        <Card.Body>
          <div style={{ height: '350px' }}>
            <Chart type="line" data={timelineData} options={timelineOptions} />
          </div>
        </Card.Body>
      </Card>
      ...
      <ListGroup className="shadow-sm mb-4 border-0">
        {stats.recent_feedback.map(f => (
          <ListGroup.Item key={f.id} className="p-4 border-0 border-bottom mb-2 rounded shadow-sm">
            <div className="d-flex justify-content-between align-items-start mb-2">
              <div>
                <Badge bg={getRatingColor(f.overall)} className="me-2 px-3 py-2">
                  {f.overall} ⭐
                </Badge>
                {f.service_name && <Badge bg="light" text="dark" className="me-2 border">{f.service_name}</Badge>}
                <span className="text-muted small">
                  {new Date(f.created_at).toLocaleDateString()}
                </span>
              </div>
            </div>
            <p className="mb-0 mt-2" style={{ fontSize: '1.1rem', color: '#444' }}>
