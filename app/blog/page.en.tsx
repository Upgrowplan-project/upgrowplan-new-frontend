...
export default function BlogPage() {
  ...
  const [staticPosts] = useState<Post[]>(staticPostsEn);
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_BLOG_URL}/api/posts`)
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          ...
          // For now, keep Russian text for dynamic posts
          // TODO: Add translation service or manual translations
    ...
    stompClient.connect({}, () => {
      ...
      stompClient.subscribe("/topic/posts", (msg) => {
        ...
        setDynamicPosts((prev) => {
          const allIds = new Set([...staticPostsEn.map(p => p.id), ...prev.map(p => p.id)]);
